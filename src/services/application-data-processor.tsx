...
export const getMostRecentApplications = async (
  applications: JobApplication[]
): Promise<JobApplication[]> => {
  return new Promise<JobApplication[]>((resolve, reject) => {
    // Take the provided application context and sort by created date in descending order
      ...
      const mostRecentApplications = applications
        .sort(
          (a: JobApplication, b: JobApplication) =>
            new Date(String(b.createdOn)).getTime() -
            new Date(String(a.createdOn)).getTime()
        )
        .slice(0, 5);
      resolve(mostRecentApplications);
    } catch (error) {
      console.error("Error processing most recent applications: ", error);
      reject(error);
    }
  });
};

export const getNumberOfApplicationsToday = async (
  applications: JobApplication[]
): Promise<number> => {
    ...
    try {
      ...
      const applicationsToday = applications.filter(
        (application: JobApplication) =>
          ...
          new Date(application.createdOn).toISOString().split("T")[0] === today
      );
      resolve(applicationsToday.length);
    } catch (error) {
      console.error(
        "Error processing number of applications created today: ",
        error
      );
      reject(error);
    }
